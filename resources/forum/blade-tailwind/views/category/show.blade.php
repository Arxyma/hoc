            ...
            @if ($category->accepts_threads)
                @can('createThreads', $category)
                    <x-forum::button-link href="{{ Forum::route('thread.create', $category) }}"
                        class="float-end">{{ trans('forum::threads.new_thread') }}</x-forum::button-link>
                @endcan
            @endif
                        ...
                        <form :action="actions[state.selectedAction]" method="POST">
                            ...
                            <div class="text-end mt-2">
                                <div class="form-check">
                                    ...
                                    <input type="checkbox" value="" id="selectAllThreads" class="align-middle"
                                        @click="toggleAll"
                                        :checked="state.selectedThreads.length == selectableThreadIds.length">
                                </div>
                            </div>
                        @endcan
                @endif
                            ...
                            <div class="bg-white shadow-sm rounded-md min-w-96 max-w-full">
                                ...
                                <div class="p-6">
                                    <div class="mb-3">
                                        <div>
                                            <x-forum::label
                                                for="bulk-actions">{{ trans_choice('forum::general.actions', 1) }}</x-forum::label>
                                        </div>
                                    ...
                                    <div class="text-end">
                                        <button type="submit" class="bg-blue-500 text-white rounded-md px-3 py-1"
                                            @click="submit"
                                            :disabled="state.selectedAction == null">{{ trans('forum::general.proceed') }}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card my-3">
                    <div class="card-body">
                        ...
                        @can('createThreads', $category)
                            <br>
                            <a
                                href="{{ Forum::route('thread.create', $category) }}">{{ trans('forum::threads.post_the_first') }}</a>
                        @endcan
                    </div>
                </div>
            @endif
            ...
            <div class="row">
                ...
                <div class="col col-xs-4 text-end">
                    @if ($category->accepts_threads)
                        @can('createThreads', $category)
                            <x-forum::button-link
                                href="{{ Forum::route('thread.create', $category) }}">{{ trans('forum::threads.new_thread') }}</x-forum::button-link>
                        @endcan
                    @endif
                </div>
            </div>
    ...
    <script type="module">
        Vue.createApp({
            setup() {
                ...
                function toggleAll() {
                    state.selectedThreads = (state.selectedThreads.length < selectableThreadIds.length) ?
                        selectableThreadIds : [];
                }

                function submit(event) {
                    if (actionMethods[state.selectedAction] === 'DELETE' && !confirm(
