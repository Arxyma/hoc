            ...
            <div class="md:w-3/6 text-center md:text-left">
                ...
                @if ($category->getImageUrl())
                    <img src="{{ $category->getImageUrl() }}" alt="{{ $category->title }}"
                        class="rounded w-24 h-24 object-cover mb-2" />
                @endif

                <h5 class="text-lg">
                    <a href="{{ Forum::route('category.show', $category) }}"
                        style="color: {{ $category->color_light_mode }};">{{ $category->title }}</a>
                </h5>
                    ...
                    @if ($category->newestThread)
                        <div>
                            <a href="{{ Forum::route('thread.show', $category->newestThread) }}"
                                class="mr-1">{{ $category->newestThread->title }}</a>
                            @include ('forum::partials.timestamp', [
                                'carbon' => $category->newestThread->created_at,
                            ])
                        </div>
                    @endif
                    @if ($category->latestActiveThread && $category->latestActiveThread->post_count > 1)
                        <div>
                            <a href="{{ Forum::route('thread.show', $category->latestActiveThread->lastPost) }}"
                                class="mr-1">Re: {{ $category->latestActiveThread->title }}</a>
                            @include ('forum::partials.timestamp', [
                                'carbon' => $category->latestActiveThread->lastPost->created_at,
                            ])
                        </div>
                    @endif
                @endif
        ...
        <div class="subcategories">
            @foreach ($category->children as $subcategory)
                <div class="bg-white -mt-1 shadow rounded-b-md">
                    <div class="flex flex-col md:items-start md:flex-row md:justify-between md:gap-4 p-6">
                        <div class="md:w-3/6 text-center md:text-left">
                            <!-- Tambahkan gambar subkategori -->
                            @if ($subcategory->getImageUrl())
                                <img src="{{ $subcategory->getImageUrl() }}" alt="{{ $subcategory->title }}"
                                    class="rounded w-24 h-24 object-cover mb-2" />
                            @endif

                            <a href="{{ Forum::route('category.show', $subcategory) }}"
                                style="color: {{ $subcategory->color_light_mode }};">{{ $subcategory->title }}</a>
                            ...
                            @if ($subcategory->newestThread)
                                <div>
                                    <a href="{{ Forum::route('thread.show', $subcategory->newestThread) }}"
                                        class="mr-1">{{ $subcategory->newestThread->title }}</a>
                                    @include ('forum::partials.timestamp', [
                                        'carbon' => $subcategory->newestThread->created_at,
                                    ])
                                </div>
                            @endif
                            @if ($subcategory->latestActiveThread && $subcategory->latestActiveThread->post_count > 1)
                                <div>
                                    <a href="{{ Forum::route('thread.show', $subcategory->latestActiveThread->lastPost) }}"
                                        class="mr-1">Re: {{ $subcategory->latestActiveThread->title }}</a>
                                    @include ('forum::partials.timestamp', [
                                        'carbon' => $subcategory->latestActiveThread->lastPost->created_at,
                                    ])
                                </div>
                            @endif
                        </div>
