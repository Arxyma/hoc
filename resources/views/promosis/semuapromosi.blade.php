        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-full">
                    ...
                    <!-- Search Form -->
                    <form action="{{ route('promosis.semuapromosi') }}" method="GET"
                            ...
                            <thead class="">
                                <tr>
                                    ...
                                    <th
                                        ...
                                        Gambar Produk</th>
                                    <th
                                        class="px-4 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-52">
                                        Nama Produk</th>
                                    <th
                                        class="px-4 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            ...
                            <tbody>
                                @forelse($promosis as $index => $promosi)
                                    <tr class="border-b dark:border-gray-900">
                                        ...
                                        <td class="px-4 py-8 flex items-center justify-center">
                                            <a href="{{ route('promosis.detail', $promosi->slug) }}" class="block">
                                                @if ($promosi->foto_produk)
                                                    ...
                                                    @else
                                                        <div
                                                            class="w-24 h-24 bg-gray-200 flex items-center justify-center text-gray-500 rounded-lg">
                                                            No Image
                                                        </div>
                                                    @endif
                                                @else
                                        ...
                                        <td class="px-4 py-2 text-center text-gray-900 dark:text-gray-200">
                                            {{ $promosi->created_at->format('d/m/Y') ?? 'Unknown' }}</td>
                                        <td class="px-4 py-2 text-center text-gray-900 dark:text-gray-200">
                                            {{ $promosi->user->name ?? 'Unknown' }}</td>
                                        @cannot('pimpinan')
                                            <td class="px-4 py-2 text-center">
                                                <form action="{{ route('promosis.destroy', $promosi->id) }}" method="POST"
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        data-title="{{ $promosi->judul }}">Delete</button>
                                                </form>
                                            </td>
                                        @endcannot
                                    </tr>
