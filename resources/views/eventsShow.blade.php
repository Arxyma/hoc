<x-app-layout>
                ...
                <div class="mb-6 w-full shrink-0 grow-0 basis-auto lg:mb-0 lg:w-6/12 lg:pr-6">
                    <div class="flex flex-col">
                        <div class="ripple relative overflow-hidden rounded-lg bg-cover bg-[50%] bg-no-repeat shadow-lg dark:shadow-black/20">
                            ...
                            <a href="#!">
                                <div class="absolute top-0 right-0 bottom-0 left-0 h-full w-full bg-[hsl(0,0%,98.4%,0.2)] opacity-0 transition duration-300 ease-in-out hover:opacity-100"></div>
                            </a>
                        </div>
    ...
    <!-- Alert untuk profil tidak lengkap -->
    @if(session('incomplete_profile'))
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                if (confirm("{{ session('message') }} Klik OK untuk melengkapi profil.")) {
                    window.location.href = "{{ route('profile.edit') }}"; // Sesuaikan dengan route edit profil
                }
            });
        </script>
    @endif
